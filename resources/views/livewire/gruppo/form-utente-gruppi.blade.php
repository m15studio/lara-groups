                                ...
                                @foreach ($tuttiGruppi as $gruppo)
                                    <div class="relative flex items-start py-4">
                                        <div class="min-w-0 flex-1 text-sm">
                                            <label for="check_{{ $gruppo->id }}"
                                                class="font-medium text-gray-700 select-none">{!! $gruppo->gruppo_nome !!}</label>
                                            @if (in_array($gruppo->id, $gruppiSelezionatiIniziali))
                                                @php($key = array_search($gruppo->id, array_column($tuttiGruppiSocio, 'id')))
                                                @if ($tuttiGruppiSocio[$key]['id'] == $gruppo->id)
                                        ...
                                        <div class="ml-3 flex items-center h-5">
                                            <input type="checkbox" wire:model="gruppiSelezionati"
                                                id="check_{{ $gruppo->id }}" value="{{ $gruppo->id }}"
